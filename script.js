...
let kucing = new Animal("Kucing", 2, true);
...
let kelinci = new Rabbits("Kelinci Anggora", 2);
...
let searchFruit = myFruits.filter((fruit) =>
  fruit.name.toLowerCase().includes("banana".toLowerCase())
);
let searchFamily = myFruits.filter((fruit) =>
  fruit.family.toLowerCase().includes("rosaceae".toLowerCase())
);